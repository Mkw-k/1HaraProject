define([
    ...
    var Util = {
        ...
        Event: {
            KEYCODE: {
                'LEFT_CLICK': 1, 'BACKSPACE': 8, 'TAB': 9, 'ENTER': 13, 'SPACE': 32, 'LEFT_ARROW': 37, 'RIGHT_ARROW': 39,
                'UP': 38, 'DOWN': 40, 'DELETE': 46, 'ZERO': 48, 'NINE': 57, 'DOT': 190
            }
        },
        Http: {
            ajax: function(url, data, type, dataType, async) {
                ...
                return $.ajax({type : type || 'get', url : url, data : data, dataType : dataType || 'json', async: async});
            }
        },
        Layer: {
            ...
            clonePosition: function($target, $source, options) {
                options = _.merge({'setTop': true, 'setLeft': true, 'setWidth': false, 'setHeight': false, 'offsetTop': 0, 'offsetLeft': 0}, options || {});
            ...
            arrangeDepthTinyScrollbar: function(fromDepth) {
                ...
                $.each($wrapSection, function() {
                    ...
                    $wrapDepthCategory = $(this).find('.area_btn').parents('.wrap_depth_category');
                    ...
                    if (typeof fromDepth === 'undefined' || fromDepth === 'depth1') {
                        if (wrapScrollDepth2.length) {
                            ...
                            if ($onEl.length) {
                                wrapScrollDepth2.data('plugin_tinyscrollbar').update($onEl.eq(0).position().top);
                    ...
                    if (wrapScrollDepth3.length) {
                        $onEl = wrapScrollDepth3.find('input[type=checkbox]:checked').filter(':visible');
                        if ($onEl.length) {
        ...
        swipe: function () { //S필터 스와이프
            ...
            try{filterSwiper = new Swiper($swipe, options);}catch(e){} //최근 검색조건이 없을경우 swipe 할 s필터값 존재x

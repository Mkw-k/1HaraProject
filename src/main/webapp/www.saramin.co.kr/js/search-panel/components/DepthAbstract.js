define([
    ...
    DepthAbstract.prototype = {
        ...
        validateCheckedCount: function ($this, isExceptionCountFlag) {
            ...
            if (this.name === 'area') {
                if (this.getCheckedCount($this) > currentValidateCount
                    && ($this.prop('checked') || !$this.prop('checked') && $this.data('is_representative') === 'n')) {
                    if ($this.is(':checked')) {
        ...
        getSelectedTitleTypeAll: function (key, code) {
            ...
            return this.$wrapper.find(selector + '_' + code).find('span.txt').text().replace(/\s$/, '') + allText;
        },
